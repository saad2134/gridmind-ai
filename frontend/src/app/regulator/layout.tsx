...
const regulatorNavItems = [
  ...
  },
  {
    ...
    url: "/regulator/reliability",
    icon: Shield,
  },
  {
    title: "Reports",
    url: "/regulator/reports",
    icon: BarChart3,
  },
  {
    title: "Alerts",
...
export default function RegulatorLayout({
  ...
  return <RegulatorSidebar>{children}</RegulatorSidebar>;
}

function RegulatorSidebar({ children }: { children: React.ReactNode }) {
  ...
  return (
    <SidebarProvider defaultOpen={true} className="h-screen">
      <Sidebar collapsible="offcanvas" className="border-r border-primary/20">
          ...
          <div className="flex items-center justify-between px-2 w-full">
            <div className="flex items-center gap-3 pt-1.5">
              ...
              <div className="flex flex-col justify-center">
                ...
                <span className="text-xs text-muted-foreground">Regulator</span>
              </div>
            </div>
          </div>
        </SidebarHeader>
        ...
        <SidebarContent>
          <SidebarGroup>
            <SidebarGroupLabel className="text-primary font-semibold mb-2">Regulator Tools</SidebarGroupLabel>
            <SidebarMenu>
            ...
            <div className="flex gap-2">
              <Link
                ...
                className={`flex-1 flex items-center gap-3 p-2 rounded-lg transition-colors border border-foreground/10 ${
                  pathname === "/regulator/profile"
                    ? "bg-primary/50 dark:bg-primary/20 border-primary dark:border-primary"
                    : "bg-muted/50 hover:bg-muted"
                }`}
              >
                <div className="w-9 h-9 rounded-full bg-orange-500 flex items-center justify-center font-semibold text-sm shrink-0">
                  RG
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-sm font-medium truncate">Regulator</p>
        ...
        <header className="flex h-16 shrink-0 items-center gap-2 border-b border-primary/20 px-4 sticky top-0 z-50 bg-background/80 backdrop-blur-md">
          ...
          <div className="flex-1">
            ...
            <p className="text-xs text-muted-foreground hidden sm:block">
              ...
              {pathname === '/regulator/compliance' && 'Monitor regulatory compliance'}
          ...
          <div className="flex items-center gap-2">
            ...
            <Sheet>
              ...
              <SheetContent>
                <SheetHeader>
                  <SheetTitle>Regulatory Alerts</SheetTitle>
                </SheetHeader>
                <div className="flex flex-col gap-4 mt-4">
                  <div className="flex gap-3 p-3 rounded-lg border">
                    ...
                    <div className="flex-1">
                      ...
                      <p className="text-xs text-muted-foreground">Q1 report due in 5 days</p>
                      <p className="text-xs text-muted-foreground mt-1">1 hour ago</p>
                    </div>
                  </div>
                  <div className="flex gap-3 p-3 rounded-lg border border-green-200 dark:border-green-800">
                    <div className="w-10 h-10 rounded-full bg-green-100 dark:bg-green-900/30 flex items-center justify-center">
                      <Shield className="w-5 h-5 text-green-500" />
                    </div>
                    <div className="flex-1">
                      <p className="text-sm font-medium">Safety Standards Met</p>
                      <p className="text-xs text-muted-foreground">All grid safety checks passed</p>
