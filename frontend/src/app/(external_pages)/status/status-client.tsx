...
export default function StatusPageClient() {
  ...
  const getStatusIcon = () => {
    if (loading) return <RefreshCw className="w-8 h-8 text-muted-foreground animate-spin" />;
    if (allOperational) return <CheckCircle2 className="w-8 h-8 text-green-500" />;
    if (hasIssues) return <AlertTriangle className="w-8 h-8 text-yellow-500" />;
